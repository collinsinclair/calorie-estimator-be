...
async def estimate_calories(food_input: FoodDescription) -> CalorieEstimateResponse:
    ...
    """
    logger.info(f"Received estimation request: {food_input.description}")

    try:
        cached_response = estimator_service.get_cached_estimate(food_input.description)
        if cached_response:
            logger.info(f"Returning cached response for: {food_input.description}")
        ...
        logger.info(f"Successfully processed request: {food_input.description}")
        return response
